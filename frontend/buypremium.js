...
async function getUser() {
    try{
        ...
        const res = await axios.get("http://3.16.152.73:3000/premium/getuser",config);
        if(res!=undefined) {
...
async function commitPayment(e) {
 try{
    const config={headers:{'Content-Type':'application/JSON',Authorization:localStorage.getItem('token')}};
    const res = await axios.get("http://3.16.152.73:3000/premium/payment", config);
    ...
    if(res!=undefined) {
        var options = {
            ...
            "handler": async function (response) {
                await axios.post("http://3.16.152.73:3000/premium/updatepaymentstatus",{
