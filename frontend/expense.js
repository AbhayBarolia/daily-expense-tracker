...
async function getUser() {
    try{
        ...
        const res = await axios.get("http://18.119.162.42:3000/expense/getuser",config);
        if(res!=undefined) {
...
async function getPremiumData(){
    try{
            ...
            const config={headers:{'Content-Type':'application/JSON',Authorization:localStorage.getItem('token')}};
            let premiumListData= await axios.get("http://18.119.162.42:3000/expense/getpremiumlist",config);
 ...
 async function getData(page,offset){
    try{
        ...
        const config={headers:{'Content-Type':'application/JSON',Authorization:localStorage.getItem('token')}};
        const res = await axios.get("http://18.119.162.42:3000/expense/getexpense/"+page+"/"+offset,config);
...
async function sendData(obj){
    try{
    const res = await axios.post("http://18.119.162.42:3000/expense/addexpense",obj); 
    return;
...
async function removeItem(e)
        ...
        if(e.target.classList.contains("delbtn"))
    {
        if(confirm("Do you want to delete this expense"))
        {
            ...
            let res= await axios.delete("http://18.119.162.42:3000/expense/delete/"+id);
            if(res.status==200){
...
async function generateReport(){
    try{
    const config={headers:{'Content-Type':'application/JSON',Authorization:localStorage.getItem('token')}};
    let reportGen= await axios.get("http://18.119.162.42:3000/expense/report",config);
...
async function getReportRecords(){
    ...
    console.log("Records");
    window.location.href = "/reportRecords.html";
